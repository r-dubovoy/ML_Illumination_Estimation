...
class Parallel:
    def __init__(self, input_shape=0, hidden_units=0, load_path = '', learning_rate = 0.01):
        print(_bcolors.BOLD + "input shape of a model is: {}".format(input_shape) + _bcolors.ENDC)
        if load_path != '':
            self.model = keras.models.load_model(load_path)
            print(_bcolors.OKGREEN + 'loaded model: {}'.format(load_path) + _bcolors.ENDC)
        else:
            input_layer = keras.layers.Input(shape=(input_shape))

            hidden_layer_x = keras.layers.Dense(hidden_units, activation = 'relu')(input_layer)
            out_layer_x = keras.layers.Dense(1)(hidden_layer_x)

            hidden_layer_y = keras.layers.Dense(hidden_units, activation='relu')(input_layer)
            out_layer_y = keras.layers.Dense(1)(hidden_layer_y)

            hidden_layer_z = keras.layers.Dense(hidden_units, activation='relu')(input_layer)
            out_layer_z = keras.layers.Dense(1)(hidden_layer_z)

            merge_layer = keras.layers.concatenate([out_layer_x, out_layer_y, out_layer_z])
            out_layer_final = keras.layers.Dense(3)(merge_layer)

            self.model = keras.Model(inputs=[input_layer], outputs=[out_layer_final])
            # self.model.compile(optimizer = 'adam', loss = 'mean_squared_error', metrics = ['accuracy'], learning_rate=learning_rate)
    ...
    def train(self, X_train, X_test, y_train, y_test, epochs):
        self.model.fit(X_train, y_train, epochs=epochs, batch_size=8, validation_data=(X_test, y_test))
